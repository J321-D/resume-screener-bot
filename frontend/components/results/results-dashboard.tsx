    ...
    >
      ...
      <div className="terms-grid">
        <motion.article className="terms-card matched-terms" variants={cardVariants}>
          ...
          <ul className="term-cloud" aria-label="Matched terms">
            {result.matched_terms.map((item) => <li tabIndex={0} title={item.category ?? undefined} key={`${item.term}-${item.category}`}>{item.term}</li>)}
          </ul>
        ...
        <motion.article className="terms-card missing-terms" variants={cardVariants}>
          ...
          <ul className="term-cloud" aria-label="Coverage opportunities">
            {result.missing_terms.map((item) => <li tabIndex={0} title={item.category ?? undefined} key={`${item.term}-${item.category}`}>{item.term}</li>)}
          </ul>
